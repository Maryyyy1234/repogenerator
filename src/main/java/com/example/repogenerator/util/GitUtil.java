...
public class GitUtil {
    ...
    public static void pullRepository(File repoDir, String username, String token) {
        try (Git git = Git.open(repoDir)) {
            git.pull()
                    .setCredentialsProvider(new UsernamePasswordCredentialsProvider(username, token))
                    .call();
            logger.info("Successfully pulled updates for repository: {}", repoDir.getName());
        } catch (Exception e) {
            logger.error("Error pulling updates for repository: {}", repoDir.getName(), e);
        }
    }

    public static void pushRepository(File repoDir, String remoteUrl, String username, String token) {
        try (Git git = Git.open(repoDir)) {
            git.push()
                    .setRemote(remoteUrl)
                    .setCredentialsProvider(new UsernamePasswordCredentialsProvider(username, token))
                    .call();
            logger.info("Successfully pushed repository: {}", repoDir.getName());
        } catch (Exception e) {
            logger.error("Error pushing repository: {}", repoDir.getName(), e);
        }
    }
